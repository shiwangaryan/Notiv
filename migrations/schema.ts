...
export const usersInAuth = pgTable("users_in_auth", {
	...
	foreignKey({
			columns: [table.id],
			foreignColumns: [users.id],
			name: "users_in_auth_id_users_id_fk"
		}).onDelete("cascade"),
]);
...
export const users = pgTable("users", {
	...
	pgPolicy("Can update own user data.", { as: "permissive", for: "update", to: ["public"], using: sql`((( SELECT auth.jwt() AS jwt) ->> 'email'::text) = email)`, withCheck: sql`((( SELECT auth.jwt() AS jwt) ->> 'email'::text) = email)`  }),
